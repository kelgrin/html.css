...
// Делаем React доступным глобально для пользовательского кода в тренажёре
...
function ReactPlayground() {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => compileAndRun(code), 500)
          ...
          <div className="p-4 min-h-[200px] bg-white">
            {output.type === 'error' ? (
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                ...
                <pre className="text-sm text-red-600 whitespace-pre-wrap font-mono">{output.message}</pre>
              </div>
            ) : output.element ? (
              output.element
            ) : (
              <div className="text-gray-400 text-center py-12">
                Напиши код, чтобы увидеть результат
              </div>
            )}
          </div>
        </div>
      ...
      <header className="bg-gradient-to-r from-sky-600 via-blue-600 to-indigo-700 text-white py-16 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <p className="text-xl text-sky-100 max-w-2xl mx-auto leading-relaxed">
            Полное руководство по React: от основ до сборки проекта, с живыми примерами и интерактивным тренажёром
          </p>
        </div>
      </header>
